...
int ReadPageTable(const char *filename, PageTable *pt)
{
	...
	for(int i=0;i<3;i++){
		...
		int tnum;
		fscanf(fp,"%s %d\n",temp, &tnum);
		if(!strcmp(temp,"BITS_FOR_PAGE_NUMBER")){
			pt->bits_for_page_number = tnum;
		} 
		
		if(!strcmp(temp,"BITS_FOR_OFFSET")){
			pt->bits_for_offset = tnum;	
		}
	 
		if(!strcmp(temp,"PAGE_TABLE_LENGTH")){
			pt->page_table_length = tnum;	
		} 	
	}
	
	pt->frame_number = (int*) malloc(sizeof(int)*(1<<pt->page_table_length));
...
int Translate(int logical_addr, PageTable *pt)
{
	...
	*/
	...
	int physical_addr =(pt->frame_number[page_number]<<2)|offset;
